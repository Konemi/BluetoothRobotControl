...
import iut.valence.tp.bluetoothrobotcontrol.MainActivity;
...
public class BTConnectionHandler {
   ...
   public void sendBtData(Context context, String commande) {
      try {
         this.sendData(commande);
      } catch (IOException e) {
         e.printStackTrace();
      } catch (BTHandlingException e) {
         Toast.makeText(context, "Failed to send data : " + commande + ".", Toast.LENGTH_LONG).show();
      }
   }
}
